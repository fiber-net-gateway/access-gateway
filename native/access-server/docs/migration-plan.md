...
本文定义将 Java `ploto-gateway/ploto-unified-access` 迁移到本仓库
`native/access-server` 的范围、工作分解、阶段门槛和完成标准。初始 C++ 实现来自
`third_party/fiber-gateway-cpp/apps/access-server` 所固定的上游版本。
...
| 能力 | C++ 复用位置 | 本次验收边界 |
| --- | --- | --- |
| runtime、HTTP、路由基础设施 | 子模块 `src/` 中的 `fiber_lib` | 只验收 access-server 最终请求结果 |
| JSON | 子模块 `src/common/json/` | 验收统一接入配置的输入行为，不比较 Jackson 实现 |
| 脚本引擎 | 子模块 `src/script/`、`src/http_script/` | 不做通用脚本语言/VM 兼容 |
| connection pool | 子模块 `src/http/*ConnectionPool*` | 不比较池算法、连接选择和内部时序 |
| Nacos | 子模块 `apps/nacos/` | 验收 access-server 的 data ID、group、更新结果 |
| CAT | 子模块 `apps/cat/` | 不比较协议编码、线程和客户端内部实现 |
| Prometheus | 子模块 `apps/prometheus/` | 不比较 Java 指标库内部实现 |
...
- [x] 创建 `native/access-server`、CMake 目标和 fail-closed 入口；
- [x] 固定首个 Java commit；
...
| Nacos/CAT/Prometheus 内部实现 | 使用固定子模块中的组件；仅验证 access-server 边界结果 |
