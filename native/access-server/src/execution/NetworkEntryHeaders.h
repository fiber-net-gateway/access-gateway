...
// proxy_set_header semantics), so direct clients cannot forge them. An empty
// entry disables injection entirely. Returns false when header allocation
// fails; the request then proceeds unmodified, which the legacy client
// metadata resolver already tolerates.
