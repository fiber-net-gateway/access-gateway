...
直连请求以业务 listener 的实际 TLS 状态判断 HTTPS；HTTP/1 exchange 不依赖请求头推断协议。
仅在 `ACCESS_SERVER_CLIENT_METADATA_MODE=trusted_proxy` 且 socket peer 命中可信 CIDR 时，
`Forwarded: proto=` 或对齐的 `X-Forwarded-Proto` 才能覆盖 external scheme。
当前业务端口没有并行的明文 HTTP 监听器：Project 的 HTTPS redirect 仅用于受信任 Ingress/LB
同时接收 HTTP/HTTPS、规范设置转发头且其地址被列入可信 CIDR 的部署路径。直连 access-server 的明文
